...
- `combat/`: pure health/target contracts, explicit deterministic target registration and selection, a timed unarmed attack state machine, and a thin Babylon/DOM presentation adapter.
- `enemies/`: centralized Roaming Zombie calibration, a pure CombatTarget-backed AI state machine, a narrow multi-agent coordinator, and procedural Babylon presentation.
...
Combat targeting is intentionally parallel to, not merged with, contextual interaction. `CombatTargetSystem` considers only explicitly registered `CombatTarget` instances, selects deterministically by distance and runtime ID, and applies a small switch bias. `MeleeCombatSystem` locks the selected target when an attack begins, schedules exactly one impact from centralized fist timing, and revalidates registration, life state, and hit range at that impact. The attack blocks movement only before impact; recovery permits movement. `HealthPool`, combat targets, and combat timing remain Babylon/DOM-free, while `CombatPresentation` owns dummy meshes, target/health UI, damage numbers, recoil, and death cleanup.

`HealthPool` is also the sole Player-health source. The Player owns a 100-point pool read directly by HUD and F2. Zero health enters a minimal terminal gate that cancels movement, combat, harvesting, and interaction input while rendering continues; it is not a production death/respawn system. `RoamingZombie` implements the unchanged `CombatTarget` contract and adds pure IDLE/CHASE/ATTACK/RECOVERY/DEAD behavior alongside it. Thus every current Enemy is a CombatTarget, but passive Combat Dummies remain CombatTargets without AI. `EnemySystem` provides current Player position/health, independent agent updates, Player damage application, and one-shot enemy cleanup. It does not select combat targets or own duplicate health.
