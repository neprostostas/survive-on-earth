...
- `ui/`: responsive LDOE-aligned HUD shells plus a presentation-only local minimap driven by existing interactable descriptors.
...
F3 freeze supplies a zero frame delta and skips player simulation for comparison frames. It does not create a second state model or mutate gameplay data. HUD feature-dependent controls remain disabled shells until their roadmap milestones; only the existing contextual action is wired to gameplay.
