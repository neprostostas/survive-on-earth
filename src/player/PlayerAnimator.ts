...
export class PlayerAnimator {
  ...
  update(delta: number, normalizedSpeed: number): void {
    ...
    const stride = strideWave * 0.68 * moving;
    ...
    this.visual.bodyPivot.position.y = Math.abs(Math.sin(this.time * 2)) * 0.018 * moving + idle * 0.009 * (1 - moving);
    this.visual.bodyPivot.rotation.x = -0.055 * moving;
    this.visual.bodyPivot.rotation.z = strideWave * 0.014 * moving;
