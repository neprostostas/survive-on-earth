...
export class PlayerVisual {
  ...
  constructor(scene: Scene) {
    ...
    const torso = MeshBuilder.CreateCapsule("Body", { height: 0.76, radius: 0.225, tessellation: 10 }, scene);
    torso.scaling.set(1.04, 1, 0.78);
    this.part("Body", torso, shirt, this.bodyPivot, new Vector3(0, 1.2, 0));
    const chest = MeshBuilder.CreateBox("JacketChest", { width: 0.43, height: 0.39, depth: 0.1 }, scene);
    ...
    const hairCap = MeshBuilder.CreateSphere("Hair", { diameter: 0.35, segments: 9, slice: 0.55 }, scene);
    ...
    for (const x of [-0.265, 0.265]) {
      const shoulder = MeshBuilder.CreateSphere("Shoulder", { diameter: 0.205, segments: 8 }, scene);
    ...
    const backpack = MeshBuilder.CreateBox("Backpack", { width: 0.33, height: 0.44, depth: 0.16 }, scene);
    ...
    this.leftArm = this.limb(scene, "LeftArm", -0.295, 1.4, skin, shirt, false);
