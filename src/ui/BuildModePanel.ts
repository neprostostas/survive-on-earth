...
export class BuildModePanel {
  ...
  private render(): void {
    ...
    this.overlay.innerHTML = `
      <div class="build-mode-panel" role="dialog" aria-label="Base builder">
        <header class="build-mode-header">
          ...
          <button type="button" class="menu-btn ghost" data-role="exit">${menuBtnLabel("close", "Exit")}</button>
        </header>
        <nav class="build-mode-tabs" role="tablist">
          <button type="button" class="build-tab ${tab === "build" ? "active" : ""}" data-tab="build" role="tab">BUILD</button>
          <button type="button" class="build-tab ${tab === "furniture" ? "active" : ""}" data-tab="furniture" role="tab">FURNITURE</button>
        </nav>
        ...
        <div class="build-mode-detail">
          ...
          <div class="build-mode-actions">
            <button type="button" class="menu-btn ghost ${demolish ? "active" : ""}" data-role="demolish">REMOVE</button>
            ...
            <button type="button" class="menu-btn primary" data-role="place" ${!demolish && (!selected || !afford) ? "disabled" : ""}>
              ${demolish ? "REMOVE" : "PLACE F"}
            </button>
          </div>
        </div>
        <p class="build-mode-hint">Floors first · walls/doors on floors · furniture on floors · G exits</p>
      </div>`;
  }
  ...
  private cellHtml(p: BuildPieceDef, inv: PlayerInventory): string {
    ...
    return `
      <button type="button" class="build-cell ${selected ? "selected" : ""} ${afford ? "affordable" : "blocked"}"
        data-piece="${p.id}" role="option" aria-selected="${selected}" title="${escapeHtml(p.title)}">
        <span class="build-cell-icon">${icon}</span>
        <span class="build-cell-name">${escapeHtml(shortName(p.title))}</span>
      </button>`;
  }
}
